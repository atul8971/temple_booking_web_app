...
const TomorrowSevasPage: React.FC = () => {
  ...
  // Filters
  const [sevaDate, setSevaDate] = useState<string>('');

  useEffect(() => {
    // Set tomorrow's date as default seva date
    const tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
    const tomorrowStr = tomorrow.toISOString().split('T')[0];
    setSevaDate(tomorrowStr);
    loadBookings();
  }, []);
  ...
  const applyFilters = () => {
    ...
    if (sevaDate) {
      filtered = filtered.filter(booking => booking.seva_date === sevaDate);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        <div className="mb-6 flex justify-between items-center print:block">
          <div>
            <h1 className="text-3xl font-bold text-gray-800 print:text-center">Tomorrow's Sevas</h1>
            <p className="text-gray-600 mt-2 print:text-center">
              {sevaDate ? `Sevas scheduled for ${formatDate(sevaDate)}` : 'View sevas scheduled for tomorrow'}
            </p>
          </div>
          <button
            onClick={handlePrint}
        ...
        {!loading && filteredBookings.length === 0 && (
          <div className="bg-white rounded-xl shadow-lg p-8 text-center text-gray-500">
            No sevas scheduled for the selected date
          </div>
        )}
...
export default TomorrowSevasPage;
